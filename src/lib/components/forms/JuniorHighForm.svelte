<!-- src/lib/components/forms/JuniorHighForm.svelte -->
<script lang="ts">
  ...
  $effect(() => {
    if (typeof window !== 'undefined' && !isSubmitting) {
      try {
        ...
        sessionStorage.setItem('jhs_enrollment_draft', encrypted);
      } catch (error) {
  ...
  onMount(() => {
    if (typeof window !== 'undefined') {
      try {
        const draft = sessionStorage.getItem('jhs_enrollment_draft');
        if (draft) {
          ...
          if (Date.now() - decrypted.timestamp < 24 * 60 * 60 * 1000) {
            ...
            sessionStorage.removeItem('jhs_enrollment_draft');
          }
        }
      } catch (error) {
  ...
  function validateStep(step: number): boolean {
    ...
    switch (step) {
      ...
      case 4: // Documents
        const hasAnyDocument = formData.hasForm10 || formData.hasPSA || 
                              formData.hasBaptismal || formData.hasGoodMoral;
        if (!hasAnyDocument) {
  ...
  function getStepTitle(step: number): string {
    switch (step) {
      ...
      case 2: return 'Guardian Information';
      case 3: return 'Academic Background';
      case 4: return 'Required Documents';
      default: return '';
    }
  }
...
<form onsubmit={handleSubmit} class="space-y-6">
  ...
  <div class="mb-8">
    ...
    <div class="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
      <div 
        class="bg-gradient-to-r from-green-500 to-green-600 h-full rounded-full transition-all duration-500 ease-out"
    ...
    {:else if currentStep === 3}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <FormField
          ...
          helper="Your grade point average from previous school"
        />
        
        <div class="space-y-4">
          <label class="flex items-center p-4 border-2 border-gray-200 rounded-lg hover:border-green-500 transition-colors cursor-pointer">
            <input
              type="checkbox"
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">Check if transferring from another school</p>
            </div>
          </label>
          ...
          <label class="flex items-center p-4 border-2 border-gray-200 rounded-lg hover:border-green-500 transition-colors cursor-pointer">
            ...
            />
            <div>
              <span class="text-sm font-medium text-gray-700">Academic Excellence Awardee</span>
              <p class="text-xs text-gray-500">With honors or academic awards</p>
            </div>
          </label>
        </div>
      </div>
    ...
    {:else if currentStep === 4}
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-green-500 transition-all cursor-pointer group">
            <input
              type="checkbox"
              bind:checked={formData.hasForm10}
              disabled={disabled || isSubmitting}
              class="rounded border-gray-300 text-green-600 focus:ring-green-500 mr-4 mt-1 h-5 w-5"
            />
            <div class="flex-1">
              <div class="flex items-center">
                <svg class="w-5 h-5 text-gray-400 mr-2 group-hover:text-green-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
    ...
    <div class="flex items-center space-x-2">
      {#each Array(totalSteps) as _, index}
        <div class={`
          ...
          ${currentStep === index + 1 ? 'w-8 bg-green-600' : 'bg-gray-300'}
        `}></div>
      {/each}
    </div>
